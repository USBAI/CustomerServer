...
@csrf_exempt
def create_klarna_payment_intent(request):
    if request.method == 'POST':
        try:
            ...
            # Step 2: Create a PaymentIntent in Stripe with Klarna as the payment method
            payment_intent = stripe.PaymentIntent.create(
                amount=int(total_cost * 100),  # Convert amount to smallest currency unit
                currency=currency,
                payment_method_types=['klarna'],  # Restrict to Klarna payment
                metadata={
                    'user_id': user_id,
            ...
            print(f"Klarna PaymentIntent created: {payment_intent.id}")
